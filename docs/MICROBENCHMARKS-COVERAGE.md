...
| ROB/乱序窗口 | x86/C86 两个冷加载之间插入精确静态展开的一微操作整数指令窗口，输出完整重叠损失曲线和连续保持的保守拐点 | 代理覆盖；可能先耗尽 ROB、scheduler、load queue、物理寄存器，且受 SMT 活跃状态影响。ARM64 明确不可用 |
...
- 中文 B/S 报告工作台、原始 JSON/CSV、指标目录、推断依据、置信度和 caveat；
  浏览器可加载并对比多份报告。
